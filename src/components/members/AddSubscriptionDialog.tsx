...
import { CreditCard, Banknote, Building, CheckSquare, Loader2, CreditCardIcon, ArrowUpRightIcon, ShieldCheckIcon } from "lucide-react";
...
export const AddSubscriptionDialog = ({ open, onOpenChange, memberId, memberName, memberEmail = "", memberPhone = "", onSubscriptionAdded }: AddSubscriptionDialogProps) => {
  ...
  return (
    <>
          ...
          {isLoading ? (
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Select value={paymentStatus} onValueChange={(value) => setPaymentStatus(value as Member["paymentStatus"])}>
                ...
                <Tabs defaultValue="cash" value={paymentMethod} onValueChange={(value) => setPaymentMethod(value as PaymentMethod)}>
                  ...
                  <TabsContent value="card">
                    <div className="grid gap-2">
                      ...
                      <Input id="cardNumber" placeholder="**** **** **** ****" maxLength={16} value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} />
                    </div>
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <div>
                        <Label htmlFor="cardExpiry">תוקף (MM/YY)</Label>
                        <Input id="cardExpiry" placeholder="MM/YY" maxLength={5} value={cardExpiry} onChange={(e) => setCardExpiry(e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor="cardCvv">CVV</Label>
                        <Input id="cardCvv" placeholder="123" maxLength={3} value={cardCvv} onChange={(e) => setCardCvv(e.target.value)} />
                      </div>
                    </div>
                    <div className="grid gap-2 mt-2">
                      <Label htmlFor="cardHolderName">שם בעל הכרטיס</Label>
                      <Input id="cardHolderName" placeholder="Israel Israeli" value={cardHolderName} onChange={(e) => setCardHolderName(e.target.value)} />
                    </div>
                    ...
                    {showInstallments && (
                      <div className="mt-4 p-3 bg-gray-50 rounded border">
                        ...
                        <RadioGroup value={installments.toString()} onValueChange={(val) => setInstallments(Number.parseInt(val))} className="flex flex-wrap gap-2 mt-2">
                  ...
                  <TabsContent value="bank">
                    <div className="grid gap-2">
                      ...
                      <Input id="bankName" placeholder="לאומי / פועלים / מזרחי וכו'" value={bankName} onChange={(e) => setBankName(e.target.value)} />
                    </div>
                    <div className="grid gap-2 mt-2">
                      <Label htmlFor="bankBranch">מספר סניף</Label>
                      <Input id="bankBranch" placeholder="123" value={bankBranch} onChange={(e) => setBankBranch(e.target.value)} />
                    </div>
                    <div className="grid gap-2 mt-2">
                      <Label htmlFor="bankAccountNumber">מספר חשבון</Label>
                      <Input id="bankAccountNumber" placeholder="123456789" value={bankAccountNumber} onChange={(e) => setBankAccountNumber(e.target.value)} />
                    </div>
                    <div className="grid gap-2 mt-2">
                      <Label htmlFor="transferReference">אסמכתא</Label>
                      <Input id="transferReference" placeholder="מספר אסמכתא להעברה" value={transferReference} onChange={(e) => setTransferReference(e.target.value)} />
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="check">
                    <div className="grid gap-2">
                      ...
                      <Input id="checkNumber" placeholder="12345" value={checkNumber} onChange={(e) => setCheckNumber(e.target.value)} />
                    </div>
                    <div className="grid gap-2 mt-2">
                    ...
                    <div className="grid gap-2 mt-2">
                      ...
                      <Input id="checkBankName" placeholder="לאומי / פועלים / מזרחי וכו'" value={bankName} onChange={(e) => setBankName(e.target.value)} />
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="hyp">
                    <div className="flex flex-col">
                      <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg mb-4 w-full">
                        <div className="flex items-start">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-blue-700 mb-2">לחץ על "הוסף מנוי" כדי לעבור למסך התשלום המאובטח של HYP.</p>
                      ...
                      <div className="flex items-center mt-4">
                        <a href="https://hyp.co.il" target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline flex items-center">
                          מידע נוסף על HYP
              ...
              <div className="flex items-center space-x-2 rtl:space-x-reverse">
                <Checkbox id="sendReceipt" checked={sendReceipt} onCheckedChange={(checked) => setSendReceipt(checked as boolean)} />
                <Label htmlFor="sendReceipt" className="text-sm">
                  שלח קבלה לטלפון של המנוי
                </Label>
              </div>
            </div>
          )}
      ...
      <EnhancedHypPaymentModal
        ...
        memberAddress="" // Add this if you have it
